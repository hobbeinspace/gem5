    ...
    bufferSize(params.buffer_size), buffer(nullptr), bufferUsed(0)
{
    ...
    DPRINTF(HelloObject, "Created the goodbye object\n");
}
...
{
    DPRINTF(HelloObject, "Processing the event!\n");
    fillBuffer();
}
...
{
    DPRINTF(HelloObject, "Saying goodbye to %s\n", other_name);
...
GoodbyeObject::fillBuffer()
{
    ...
    if (bufferUsed < bufferSize - 1) {
        ...
        DPRINTF(HelloObject, "Scheduling another fillBuffer in %d ticks\n",
                bandwidth * bytes_copied);
        schedule(event, curTick() + bandwidth * bytes_copied);
    } else {
        DPRINTF(HelloObject, "Goodbye done copying!\n");
